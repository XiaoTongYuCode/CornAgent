...
`server/app/agent/outcomes.py` 仅从已声明工具的结构化执行回执生成状态与资源变化。该 metadata 随 `tool_call` 内容块持久化并发布 SSE，聊天页与嵌入面板共用操作卡；模型请求从同一份数据获得已提交、草稿、失败与未知计数。工具传输成功和模型文本不会升级为业务写入事实。扩展契约见[本轮操作与执行事实](operation-evidence.md)。
