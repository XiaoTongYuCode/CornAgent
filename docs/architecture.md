...
内置 `ask_user`、`read_file`、明确标注模拟资料的 `mock_web_search` 及五个 Root 专用子任务编排工具。工具在 `server/app/agent/tools` 注册，由统一执行器处理调用与结果。`read_file` 只读取当前会话的 PDF，私有正文在模型请求期间物化，持久状态保存引用与脱敏结果。
