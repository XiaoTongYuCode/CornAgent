...
`GET /api/v1/agent/usage?days=30` 汇总当前部署数据库中全站所有用户的数据，跨 tenant 与 membership，包含仍保留的共享工作区历史；不同用户看到相同统计。接口保留现有身份认证，但不按个人身份筛选，客户端不能指定查询用户。只返回聚合指标及工具、模型名称，不返回用户标识、会话标识或对话内容；会话、消息、附件与 SSE 继续按用户隔离。
...
验证覆盖时间范围、补零、无用量、跨用户与跨 tenant 汇总、身份切换一致性、私有会话隔离、删除联动、可选采集器、队列拥塞、采集异常、真实运行模型/工具接入。统计页面与本地预览均连接项目真实服务；无数据时显示空态，不填充示例数据。自动化测试中的合成数据只用于验证。
