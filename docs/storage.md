...
没有 base64 入库，没有对象 URL/存储路径进入模型或 SSE，没有 `read_file` 原文进入 checkpoint。
...
CornAgent 使用固定工作区，所有浏览器共享同一 scope。文件隔离到 Session，工具不能跨 Session 读取。
公网多用户访问必须由宿主产品提供访问控制，不能把固定 scope 当作多租户鉴权。
