...
export function PromptFileTray({
  files,
  onRemove,
  onRetry,
}: PromptFileTrayProps) {
      ...
      <div className="prompt-file-tray" aria-label={t('pendingFiles')}>
        {files.map((file) => (
          <div
            ...
            data-kind={
              !file.mimeType.startsWith('image/') ? 'document' : 'image'
            }
          ...
          >
            ...
            {(file.status === 'uploading' || file.status === 'processing') && (
              <span className="prompt-file__uploading" aria-hidden="true">
                <LoaderCircle className="prompt-file__spinner" size={22} />
              </span>
            )}
            {file.status === 'error' && (
              <span
                className="prompt-file__status"
                title={
                  file.error
                    ? localizeSystemMessage(file.error, locale)
                    : undefined
                }
              >
                {t('processingFailed')}
